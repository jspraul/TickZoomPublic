...
namespace SvnPostCommitHook
{
	public class CommitInformation
	{
        ...
        {
            if (svnPath.StartsWith("branches/2.0/"))
                return "Corsavy rev {0}, {1}";
			if (svnPath.StartsWith("branches/2.1/"))
                return "Serralongue rev {0}, {1}";
            else if (svnPath.StartsWith("trunk/"))
                return "Montferrer rev {0}, {1}";
