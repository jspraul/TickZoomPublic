	...
	#region Rendering Methods
		...
		public void AxisChange( Graphics g )
		{
			...
			if ( this.CurveList.IsPieOnly )
			{
				...
				//this.Legend.PositionChange = LegendPos.TopCenter;
			}
...
//			minSpaceT = Math.Max( minSpaceT, spaceT );
	...
	#region General Utility Methods
		...
		public bool FindNearestObject( PointF mousePt, Graphics g, 
			out object nearestObj, out int index, Predicate<object> predicate)
		{
			nearestObj = null;
			index = -1;
			...
			if ( AxisRangesValid() )
			{
				...
				// intersected as well.
				if ( this.GraphObjList.FindPoint( mousePt, this, g, scaleFactor, out index, predicate ) )
				{
			...
			foreach ( CurveItem curve in targetCurveList )
			{
				...
				else if ( curve.IsVisible )
				{
					...
					if ( points != null )
					{
						...
						if ( baseAxis._scale.IsAnyOrdinal && !curve.IsOverrideOrdinal ) {
							if( isXBaseAxis) {
								minOrdinal = (int) (xAct - 20);
								maxOrdinal = (int) (xAct + 20);
							} else {
								minOrdinal = (int) (yAct - 20);
								maxOrdinal = (int) (yAct + 20);
							}
						} else {
							if( isXBaseAxis) {
								minScale = xAct - 20 / xPixPerUnit;
								maxScale = xAct + 20 / xPixPerUnit;
							} else {
								minScale = yAct - 20 / yPixPerUnitAct;
								maxScale = yAct + 20 / yPixPerUnitAct;
							}
						}
						minOrdinal = Math.Max(minOrdinal,0);
						for ( int iPt = minOrdinal; iPt < curve.NPts && iPt < maxOrdinal; iPt++ )
						{
							...
							if( !baseAxis._scale.IsAnyOrdinal || curve.IsOverrideOrdinal) {
								if( isXBaseAxis) {
									if( xVal < minScale || xVal > maxScale) {
										continue;
									}
								} else {
									if( yVal < minScale || yVal > maxScale) {
										continue;
									}
								}
							}
