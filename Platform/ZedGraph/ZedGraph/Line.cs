	...
	#region Rendering Methods
                                ...
                                CurveItem curve, float scaleFactor )
		{
			...
			if ( this.IsVisible && !this.Color.IsEmpty && points != null &&
				BuildPointsArray( pane, curve, out count ) &&
				count > 2 )
			{
				float tension = _isSmooth ? _smoothTension : 0f;

				PointF[] tempPoints = new PointF[count];
				Array.Copy(arrayPoints,tempPoints,count);
				...
				if ( this.Fill.IsVisible )
				{
					...
					{
						path.AddCurve( tempPoints, 0, count - 2, tension );
						double yMin = yAxis._scale._min < 0 ? 0.0 : yAxis._scale._min;
						CloseCurve( pane, curve, tempPoints, count, yMin, path );
				...
				if ( _isSmooth )
				{
					...
					{
						// Stroke the curve
						g.DrawCurve( pen, tempPoints, 0, count - 2, tension );
				...
				if ( points != null && !_color.IsEmpty && this.IsVisible )
				{
					...
					if( baseAxis.Scale.IsAnyOrdinal) {
						...
						if( pt != null) {
							...
							if( pt != null) {
								double finalX = pt.X;
								double scalePerOrdinal = (finalX - firstX) / curve.Points.Count;
								minOrdinal = (int) Math.Max(0,(minScale - firstX) / scalePerOrdinal);
								maxOrdinal = (int) ((maxScale - firstX ) / scalePerOrdinal);
		...
		bool isOptDraw = true;
		Dictionary<long,bool> isPixelDrawn = null;
		private long GetHashCode( int x, int y) {
			return (long) x * (long) int.MaxValue + (long) y;
		}
		...
		public bool BuildPointsArray( GraphPane pane, CurveItem curve, out int count )
		{
			count = 0;
			IPointList points = curve.Points;
			...
			if ( this.IsVisible && !this.Color.IsEmpty && points != null )
			{
				...
				int pointsCapacity = ( _stepType == ZedGraph.StepType.NonStep ? 1 : 2) * (maxX * maxY) + 1;
				if( arrayPoints == null || pointsCapacity > arrayPoints.Length ) {
					arrayPoints = new PointF[( _stepType == ZedGraph.StepType.NonStep ? 1 : 2 ) *
				                       (maxX * maxY) + 1];
				}
				...
				if( baseAxis.Scale.IsAnyOrdinal && !curve.IsOverrideOrdinal) {
					...
					StockPt pt = points[0] as StockPt;
					if( pt != null) {
						double firstDate = pt.X;
						pt = points[points.Count-1] as StockPt;
						if( pt != null) {
				...
				// Skip the first point to be used later to clip to
				// the edge of the rectangle.
				index++;
				bool isFirstGoodPoint = true;
					...
					if ( !points[i].IsInvalid )
					{
						...
						if( curX < minX || curX > maxX ||
						   curY < minY || curY > maxY ) {
							...
							if( isPixelDrawn.TryGetValue(GetHashCode((int)curX,(int)curY),out value ) ) {
								continue;
							}
					     	isPixelDrawn.Add(GetHashCode((int)curX,(int)curY),true);
						}
						...
						{
							arrayPoints[index].X = curX;
							arrayPoints[index].Y = curY;
						}
						else if ( this.StepType == StepType.ForwardStep ||
										this.StepType == StepType.ForwardSegment )
						{
							arrayPoints[index].X = curX;
							arrayPoints[index].Y = lastY;
							index++;
							arrayPoints[index].X = curX;
							arrayPoints[index].Y = curY;
						}
						...
						if( isFirstGoodPoint) {
							isFirstGoodPoint = false;
							if( i>0) {
								arrayPoints[0].X = lastX;
								arrayPoints[0].Y = lastY;
							}
						}
				...
				// Make sure there is at least two valid points
				if ( index < 1 )
					return false;

				// Add an extra point at the end, since the smoothing algorithm requires it
				arrayPoints[index] = arrayPoints[index - 1];
				index++;
									...
									int count, double yMin, GraphicsPath path )
		{
			...
			if ( pane.LineType != LineType.Stack )
			{
				...
				path.AddLine( arrPoints[0].X, yBase, arrPoints[0].X, arrPoints[1].Y );
				path.AddLine( arrPoints[0].X, arrPoints[1].Y, arrPoints[1].X, arrPoints[1].Y );
			}
