...
namespace ZedGraph
{
	...
	#region Fields
		...
		/// <summary>
		/// Private field that saves the list of TradeSignal.Positions used to create the
		/// <see cref="LinearGradientBrush"/> in the constructor.  This is used strictly
		/// for serialization.
		/// </summary>
	...
	#region Constructors
		...
		/// and TradeSignal.Positions along the gradient.</param>
		...
		/// <param name="colors">The array of <see cref="Color"/> objects that defines the colors
		/// along the gradient.</param>
		/// <param name="TradeSignal.Positions">The array of floating point values that defines the color
		/// TradeSignal.Positions along the gradient.  Values should range from 0 to 1.</param>
	...
	#region Methods
		...
		/// to be filled</param>
//			Bitmap bgBuffer = null;
//			if( bgBuffer == null ||
//				rect.Width != bgBuffer.Width ||
//		    	rect.Height != bgBuffer.Height ) {
//				bgBuffer = new Bitmap((int)rect.Width,(int)rect.Height);
//				using( Graphics g = Graphics.FromImage(bgBuffer) ) {
//				}
//		    }
//			g_out.DrawImage(bgBuffer, rect);
