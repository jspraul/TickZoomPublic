...
namespace ZedGraph
{
	...
	#region Fields
		...
		/// <summary>
		/// Pixel TradeSignal.Positions at the minimum and maximum value for this scale.
...
//			_ownerAxis.DrawMinorTics( g, pane, baseVal, shiftPos, scaleFactor, topPix );
		...
		{
			double basisVal = _min;
			float val = Math.Abs( Transform( basisVal + clusterScaleWidth ) -
					Transform( basisVal ) );
			return val;
		}
	...
	#region Scale Picker Methods
		...
		protected static double CalcStepSize( double range, double targetSteps )
		{
			...
			double magMsd = ( Math.Truncate( tempStep / magPow + .5) );
