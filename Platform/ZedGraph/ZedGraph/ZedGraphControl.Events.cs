...
namespace ZedGraph
{
	...
	#region Events
		...
		public virtual event ZoomEventHandler ZoomEvent;
	...
	#region Mouse Events
		...
		protected void ZedGraphControl_MouseMove( object sender, MouseEventArgs e )
		{
			if ( _masterPane != null )
			{
				...
				else if ( _isShowDrawObjectTags)
					HandleDrawObjectTags( mousePt );
				//Revision: JCarpenter 10/06
				else if ( _isSelecting )
					HandleZoomDrag( mousePt );
			}
		...
		private Point HandlePointValues( Point mousePt )
		{
			...
			using ( Graphics g = this.CreateGraphics() )
			{
				...
				if ( _masterPane.FindNearestPaneObject( mousePt,
					g, out pane, out nearestObj, out iPt, null ) )
				{
		...
		private Point HandleDrawObjectTags( Point mousePt )
		{
			int iPt;
			GraphPane pane;
			object nearestObj;
			...
			using ( Graphics g = this.CreateGraphics() )
			{
				GraphPane.Default.NearestTol = 7.00f;

				if ( _masterPane.FindNearestPaneObject( mousePt,
				                                       g, out pane, out nearestObj, out iPt, x => { return x is ArrowObj; } ))
				{
					if ( nearestObj is ArrowObj && iPt >= 0 )
					{
