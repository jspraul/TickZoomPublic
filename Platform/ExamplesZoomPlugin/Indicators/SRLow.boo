...
class SRLow(IndicatorCommon):
	...
	override def OnIntervalClose() as bool:
		if Count == 1:
			...
			if (Bars.Low[0] < Formula.Lowest(Bars.Low, 3, 1)) and ((highestHigh - Bars.Low[0]) > minimumMove):
				highestHigh = Bars.High[0]
				lowestLow = Bars.Low[0]
			
			// when low drops below highest far enough, it's a new pivot
			if (self[0] != lowestLow) and ((Bars.High[0] - lowestLow) > minimumMove):
				self[0] = lowestLow
