...
namespace TickZoom
{
	...
	/// </summary>
	public class SRLongIntraday : Strategy
	{
		int range = 80;
		...
		public override bool OnIntervalClose(Interval timeFrame)
		{
			if( timeFrame.Equals(Intervals.Minute1)) {
				...
				if( Next.Position.Current == 0) {
					if( Position.IsFlat && Formula.CrossesOver( Bars.High, Weeks.High[1])) {
						Orders.Enter.ActiveNow.SellMarket();
					}
					if(  Position.IsFlat && Bars.Low[0] < Days.Low[1]) {
